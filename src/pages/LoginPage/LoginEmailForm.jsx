...
const LoginEmailForm = ({ setModalError, setEmail }) => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            if (!currentUserEmail)
                throw new Error(
                    "LoginEmailForm Error: Error while fetching sign in methods for email."
                );
            if (!currentUserEmail.includes("password")) {
                setModalError(true);
                throw new Error(
                    "LoginEmailForm Error: No sign in methods available for this email"
                );
            }
            setModalError(false);
            setEmail(data.email);
